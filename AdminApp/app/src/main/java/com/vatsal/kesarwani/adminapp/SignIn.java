...
public class SignIn extends AppCompatActivity {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            sharedPreferences.edit()
                                    .putString(AppConfig.PASSCODE,spass)
                                    .apply();
                            startActivity(new Intent(getApplicationContext(),Request.class));
                        } else {
                            // If sign in fails, display a message to the user.
                            Log.w(TAG, "signInWithCredential:failure", task.getException());
                            Toast.makeText(SignIn.this, "Failed "+task.getException(), Toast.LENGTH_SHORT).show();
                        }
    ...
    private boolean check(){
        spass=passcode.getText().toString();
        return spass.equals("#123456");
    }

    private void init() {
        passcode=findViewById(R.id.passcode_new);
